...
/**
 * One qualifying question per screen, advanced with the OK button.
 *
 * An option can be picked with its letter key, and Enter moves on once
 * something is selected — so the whole set can be answered without the mouse.
 */
export function Quiz({ onComplete }: { onComplete: (answers: QuizAnswers) => void }) {
  ...
  const choose = useCallback(
    (value: string) => setAnswers((prev) => ({ ...prev, [question.id]: value })),
    [question.id],
  );
  ...
  }, [question.options, choose, advance]);
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="mt-5 flex items-center gap-3">
            <button
              ...
              onClick={advance}
              disabled={!selected}
              className="rounded-lg px-6 py-2.5 text-sm font-extrabold tracking-wide text-white shadow-[0_0_20px_rgba(56,163,184,0.3)] transition-all duration-200 hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none sm:text-base"
              style={{
                background:
            ...
            <span className="text-[10px] text-white/35 sm:text-xs">
              press <span className="font-bold text-white/55">Enter ↵</span>
            </span>
            ...
            {index > 0 ? (
              <button
                ...
                className="ml-auto text-xs font-medium text-white/45 underline-offset-4 transition-colors hover:text-white/80 hover:underline sm:text-sm"
              >
                Back
              </button>
            ) : null}
