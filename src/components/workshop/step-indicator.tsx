...
export const WORKSHOP_STEPS = ['Snabbtest', 'Dina uppgifter', 'Boka samtal'] as const;

export function StepIndicator({ currentStep }: { currentStep: 1 | 2 | 3 }) {
  return (
    <div className="mx-auto mb-6 max-w-2xl">
      <div className="grid w-full grid-cols-3 overflow-hidden rounded-2xl border border-[#2f343a]/70 bg-[#0f1113]/95">
        {WORKSHOP_STEPS.map((label, index) => {
          ...
          return (
            <div
              key={label}
              className={cn(
                'relative flex min-h-16 flex-col items-center justify-center border-r border-[#2f343a]/40 px-1.5 py-2 text-center last:border-r-0 sm:min-h-24 sm:px-4 sm:py-3',
                isActive && 'bg-[#1e2a12]/85',
              )}
            >
              <span
                className={cn(
